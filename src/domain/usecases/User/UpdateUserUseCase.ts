...
export class UpdateUserUseCase {
    ...
    async execute(id: number, data: Partial<{ username: string; password: string; rol: UserRole; employeeId?: number | null; }>): Promise<User | null> {
        if (data.password && !this.validatePassword(data.password)) {
            ...
            if (data.password && !this.validatePassword(data.password)) {
                ...
                throw new Error('La contraseña no cumple con los requisitos de robustez.');
            }
        }
