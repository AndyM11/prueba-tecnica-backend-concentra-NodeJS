...
export const listLocationsUseCase = new GetAllLocationsUseCase(locationRepo); // Solo exportar para pruebas unitarias
export const createLocationUseCase = new CreateLocationUseCase(locationRepo); // Solo exportar para pruebas unitarias
export const getLocationByIdUseCase = new GetLocationByIdUseCase(locationRepo); // Solo exportar para pruebas unitarias
export const updateLocationUseCase = new UpdateLocationUseCase(locationRepo); // Solo exportar para pruebas unitarias
export const deleteLocationUseCase = new DeleteLocationUseCase(locationRepo); // Solo exportar para pruebas unitarias
...
export const getLocations = async (req: Request, res: Response) => {
    try {
        ...
        res.status(500).json({ error: 'Error obteniendo ubicaciones', details: error instanceof Error ? error.message : error });
    }
};

export const createLocation = async (req: Request, res: Response, next: Function) => {
    try {
...
export const getLocationById = async (req: Request, res: Response) => {
    try {
        ...
        res.status(500).json({ error: 'Error obteniendo ubicación', details: error instanceof Error ? error.message : error });
    }
};
...
export const updateLocation = async (req: Request, res: Response, next: Function) => {
    try {
        ...
        const location = await updateLocationUseCase.execute(Number(id), parse.data);
        res.json(location);
    } catch (error: any) {
        next(error);
    }
...
export const deleteLocation = async (req: Request, res: Response, next: Function) => {
    try {
        const { id } = req.params;
        if (!id || isNaN(Number(id))) {
            return res.status(400).json({ error: 'ID de ubicación inválido' });
