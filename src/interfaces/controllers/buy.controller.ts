...
export const createBuy = async (req: Request, res: Response, next: Function) => {
    try {
...
export const updateBuy = async (req: Request, res: Response, next: Function) => {
    try {
        const parse = buySchema.partial().safeParse(req.body);
        if (!parse.success) {
            return res.status(400).json({ error: 'Datos inválidos', details: parse.error.issues });
        }
        const result = await updateBuyUseCase.execute(Number(req.params.id), parse.data);
        res.json(result);
    } catch (error) {
        next(error);
    }
...
export const deleteBuy = async (req: Request, res: Response, next: Function) => {
    try {
